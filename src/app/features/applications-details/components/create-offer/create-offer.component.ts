...
@Component({
  ...
  imports: [CustomInputComponent, ReactiveFormsModule, FormsModule, CustomSelectComponent, FormErrorMessageComponent],
...
export class CreateOfferComponent {
  ...
  @Output() offer = new EventEmitter<Offer | null>();
  ...
  public form = this._fb.group({
    purchased_amount: ['', [Validators.required, Validators.min(200), Validators.max(1000000), onlyNumbersValidator()]],
  ...
  public submit(): void {
    ...
    const offer = {
      ...
      payment_plan_duration: this.form.value.payment_plan_duration && Number(this.form.value.payment_plan_duration),
      payment_plan: this.form.value.payment_plan,
    } as Offer;

    this.loading.set(true);
    this.applicationsService
      .createOffer(this.app.application()!.id, this.notificationId, offer)
      .pipe(finalize(() => this.loading.set(false)))
      .subscribe(() => this.offer.emit({ ...offer, status: 'ON_HOLD' }));
  }
}
