...
const DEFAULT_ERRORS = {
  ...
  email: () => 'Invalid email.',
  minlength: (error: any) => `This field must be at least ${error.requiredLength} characters or items long.`,
  maxlength: (error: any) => `This field must be at most ${error.requiredLength} characters or items long.`,
  ...
  COMPANY_MAX_FILE_PER_TYPE: (max: number) => `Maximum number of documents reached (${max}) per type`,

  CONTACT_MAX_FILE_PER_TYPE: (max: number) => `Maximum number of documents reached (${max}) per type`,
};
